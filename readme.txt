PCSX-ReARMed-PB - port of yet another PCSX fork

http://catalystg.blogspot.ca/p/pcsx-rearmed-pb.html
-------------------------------
PCSX ReARMed PB is a port of the PCSX-ReARMed project for the BlackBerry
PlayBook. QNX specific changes were made to the MIPS->ARM recompiler,
audio driver, display driver and GUI. 

It was based off the Maemo platform of PCSX-ReARMed. It makes use of 
the ARM NEON GPU by Exophase, and hasn't been tested with other plugins.

Big thanks goes to notaz for help with porting the core emulator and 
jtokarchuk for added features, the icon and splash screen.

Usage
-----
This version has a dialog menu to pick the ISO to play. A swipe down
gesture while in game will allow you to switch to a new ISO.
...
shared/misc/pcsx-rearmed-pb/bios/
then select the BIOS you want to use in 
shared/misc/pcsx-rearmed-pb/cfg/pcsx.cfg

Plugins
-------

GPU (graphics) and SPU (sound) plugins:
